...
int dpad[] = {16, 17, 5, 19, 18}; //     center,right,left,down,up
int lb = 2;
int rb = 4;
...
void testIp()
{
  ...
  {
    display.solidC(0, 0, 0, 0, 0);
  }
  if (digitalRead(button.dpad[3]) == 0)
  {
    display.frameM[display.center[0]][display.center[1] + 1] = CRGB(0, 100, 0);
  }
  else
  {
    display.frameM[display.center[0]][display.center[1] + 1] = CRGB(0, 0, 0);
  }
  if (digitalRead(button.dpad[4]) == 0)
  {
    display.frameM[display.center[0]][display.center[1] - 1] = CRGB(0, 100, 0);
  }
  else
  ...
  if (digitalRead(button.dpad[0]) == 0)
  {
    display.frameM[display.center[0]][display.center[1]] = CRGB(0, 100, 0);
  }
  else
  {
    display.frameM[display.center[0]][display.center[1]] = CRGB(0, 0, 0);
  }
  if (digitalRead(button.dpad[1]) == 0)
  {
    display.frameM[display.center[0] + 1][display.center[1]] = CRGB(0, 100, 0);
  }
  else
  {
    display.frameM[display.center[0] + 1][display.center[1]] = CRGB(0, 0, 0);
  }
  if (digitalRead(button.dpad[2]) == 0)
  {
    display.frameM[display.center[0] - 1][display.center[1]] = CRGB(0, 100, 0);
  }
  else
